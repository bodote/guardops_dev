...
import RAGPrompting from "@/components/Prompting/RAGPrompting";
import {
  DivisionIcon,
  DownIcon,
  RightIcon,
...
const Index = () => {
  ...
  const renderContent = () => {
    switch (currentView) {
      case 'rag':
        return <RAGPrompting onBack={() => setCurrentView('menu')} />;
      case 'generate':
            ...
            <div className="grid gap-6 md:grid-cols-3 sm:grid-cols-2 grid-cols-1">
              ...
              >
                <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center group-hover:bg-blue-200 transition-colors">
                  <BiBrain className="text-blue-600 text-2xl" />
                </div>
                <h3 className="font-Archivo font-medium text-lg text-center">
                  Optimize Knowledge-Based Prompts
                </h3>
                <p className="text-sm text-gray-600 text-center">
                  Create tailored prompts for your knowledge-driven applications
                </p>
              </button>
