...
const CustomAPIEndpoint = ({
  ...
  return (
    <div className="mt-[31px]">
      ...
      <div className="flex items-center sm:gap-[22px] gap-[0px] flex-wrap">
        <div className="sm:w-[360px] w-full relative">
          ...
          <p className="text-[#68727D] font-normal text-[14px] font-Inter">
            Enter the API-Key for Playground Tracing.
          </p>
          <input
            value={apiEndpint}
            onChange={(e) => setApiEndpoint(e.target.value)}
            className="h-10 border border-[#EAEBF0] my-[6px] rounded w-full focus:ring-0 focus:outline-none focus:!border-[#EAEBF0] font-normal text-[15px] font-Inter pr-[32px] p-[8px_32px_8px_12px]"
            required=""
          />
        </div>
        <button
      ...
      <p className="text-[#68727D] font-normal text-[14px] font-Inter">
        Enter the API-Endpoint for Custom-Provider
      </p>
    </div>
  );
};
